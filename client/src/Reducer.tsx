...
// work from above
// 1.add your ActionType
// 2.define it and apply database to it
...
export function appReducer(state: AppState, action: DispatchAction): AppState {
	...
	const newState = (() => {
		switch (type) {
			...
			// other set methods
			// case "SET_artists": {
			// 	const artists = args.get("artists");
			// 	return state.set("artists", artists);
			// }
			// case "SET_ALL_SONGS": {
			// 	const songs = args.get("allSongs");
			// 	return state.set("allSongs", songs);
			// }
			case "PLAY_SONG": {
				const notes = state
					.get("songs")

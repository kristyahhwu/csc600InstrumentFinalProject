...
function SongsNav({ state, dispatch }: SideNavProps): JSX.Element {
	...
	// create
	// const allSongs: List<any> = state.get("allSongs", List());
	// const artists: List<any> = state.get("artists", List());

	const songs: List<any> = state.get("songs", List());

	// console.log("Songs: ", allSongs.toJS());
	// console.log("artists: ", artists.toJS());
	...
	return (
		<div>
			...
			{/* <button
				className="allSongs-button" // to show all songs
				onClick={() => {
					dispatch(new DispatchAction("SET_SONGS", { songs: allSongs }));
				}}
			>
				All Songs
			</button> */}
			...
			{/* <Section title="Artists">
				<div style={{ height: "10rem" }}>
					{artists.map((artist) => (
						<div
							key={artist.get("artist_id")}
							className="f6 pointer underline flex items-center no-underline i dim"
							onClick={() => {
								const updatedSong = allSongs.filter(
									(song) => song.get("artist") === artist.get("artist_id")
								);
								dispatch(
									new DispatchAction("SET_SONGS", { songs: updatedSong })
								);
								console.log(artist.get("artist_id"));
							}}
						>
							<Music20 className="mr1" />
